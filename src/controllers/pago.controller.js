    ...
    for (const s of servicios) {
      if (s.precio == null || isNaN(s.precio) || Number(s.precio) < 0) {
        return res.status(400).json({
          message: "Cada servicio debe tener un precio numérico igual o mayor a 0.",
        });
      }
    }

    const ultimoPago = await Pago.findOne().sort({ numeroBoleta: -1 });

    let nuevoNumero = 224; // valor inicial
